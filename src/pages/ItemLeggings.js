...
function ItemLeggings() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8080/leggings/${id}`)
      .then((response) => {
  ...
  console.log("Slides in ItemLeggings:", slides);

  // const isItemInWishlist = wishlist.some((item) => item.id);
  ...
  const handleAddToCart = () => {
    if (selectedSize) {
      // Check if user and item are not null or undefined
      if (user && item) {
